...
Class Useraccounts_model extends CI_Model {
	...
	function changePass($table,$id,$data)
	{
		if($data['new_pass'] == $data['ver_pass'])
		{
			$query = $this->db->where('id',$id);
			// $query = $this->db->where('password', $data['old_pass']);
			$query = $this->db->set('password', "PASSWORD('".$data['new_pass']."')", FALSE);
			return $query = $this->db->update($table);
		}
		else
		{
