...
<?php echo form_open('accounting/printVoucherByDoctor');?>
		...
		foreach($accounts as $key => $value)
		{
			foreach($patients[$key] as $pkey => $pvalue)
			{
				...
				else
				{
					...
					if($value['middlename'] == 'N/A')
					{
						$mid = ' ';
					}
					else
					{
						$mid = $value['middlename'].'. ';
					}
					$name = $value['type'].'. '.$value['firstname'].' '.$mid.$value['lastname'];
					$sel_multi = form_checkbox(array('name'=>'sel_multi[]','id'=>'sel_multi','class'=>'sel_multi','value'=>$pvalue['code']));

					$this->table->add_row($count++.'.'.$sel_multi,$name,$value['vendor_account'],$pvalue['code'],mdate('%M %d, %Y',mysql_to_unix($pvalue['claims_dateofrecieve'])),'','',$pvalue['patient_name'],
