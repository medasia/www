...
<?php
	...
	foreach($op_topsheet as $hkey => $hvalue)
	{
		$this->table->set_template($tmpl);
		foreach($op_topsheet_details as $lkey => $lvalue)
		{
	...
	$count=1;
	foreach($op_topsheet as $value => $key)
	{
		if($key['availment_type'] == 'In-Patient')
		{
			unset($value);
		}
		else
		{
			$reprint = anchor(base_url().'claims/reprintOPSummary/'.$key['invoice_number'],'Reprint',array('target'=>'_blank','class'=>'btn btn-danger btn-sm'));
