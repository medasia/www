...
foreach ($operations_er_card as $value => $key)
{
	...
	if($key['patient_id'] == '0')
	{
		$action = anchor(base_url().'operations/regcard/'.$key['id'],'Register');
	...
	{
		$action = anchor(base_url().'operations/avail/'.$key['id'],'Availments');
	...
	$this->table->add_row($count++.'.'.$selMulti,$key['card_number'],$key['pin_number'],$key['amount'], $key['patient_name'], mdate('%M %d, %Y', mysql_to_unix($key['birth_date'])),
					...
					mdate('%M %d, %Y', mysql_to_unix($key['datevalid'])),mdate('%M %d, %Y', mysql_to_unix($key['dateexpiration'])).$expire_status[$value],$key['remarks'],$action);
}
