...
<?php
	...
	$this->table->set_heading('','LOA Code','Patient Name','Company - Insurance','Hospital Name','Hospital Branch','Chief Complaint/Diagnosis', 'Date','Time','Running Bill','Caller Name','Status','History','Time Generated/Updated','User','');
	$count = 1;
	...
	foreach($monitoring as $value => $key)
	{
		...
		else
		{
			...
			$edit = anchor(base_url().'verifications/editMonitoring/'.$key['code'].'/'.$key['time_generated'],'Edit',array('class'=>'btn btn-xs btn-warning','target'=>'_blank'));
