<html>
<head>
	<title>View Admission Report</title>
</head>
<h2> View Admission Report for <?php echo $codeLOA;?> of<br><?php echo $patient_name;?></h2>
<?php
	foreach($admission as $ivalue => $ikey)
	{
		...
		{
			$stmpl = array(
						'table_open' => '<table border="0" cellpadding="1" cellspacing="0" class="table table-bordered">',
						'table_close' => '</table>'
						);
	...
	foreach($admission as $value => $key)
	{
		...
		else
		{
			...
			$this->table->add_row($count++.'.',$key['code'],$key['patient_name'],$key['company_name'].' - '.$key['insurance_name'],$key['hospital_name'],$key['hospital_name'],$diagnosis[$value],
				mdate('%M %d, %Y', mysql_to_unix($key['date_admitted'])),$key['physician'],$spclst[$value],$insurance[$value],$key['history'],$key['remarks'],$key['user'],mdate('%M %d, %Y %h:%i %A',mysql_to_unix($key['time_generated'])),$edit);
		}
	}
