...
<html lang="en">
	...
	<body>
		...
		<?php
			...
			$this->table->set_heading('', 'Company Name', 'Code','Insurance','Members Count','Broker Name','Notes','Effective Date','Validity Date');
			$count=1;
			foreach ($company as $value => $key) {
				...
				$this->table->add_row(
										$count++.".".$selMulti,
										anchor(base_url()."records/accounts/view/company/".$key['id']."/", $key['name'], array('target'=>'__blank')),
										$key['code'],
										$key['comp_ins'][0]['insurance'],
										$members,
										$key['comp_ins'][0]['broker_name'],
										$key['comp_ins'][0]['notes'],
										mdate('%M %d, %Y', mysql_to_unix($key['comp_ins'][0]['start'])),
										mdate('%M %d, %Y', mysql_to_unix($key['comp_ins'][0]['end'])));
			}
