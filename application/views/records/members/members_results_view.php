...
$this->table->set_heading('', 'Name', 'Company - Insurance', 'Membership Status', 'Date of Birth', 'Age', 'Level/Position', 'Declaration Date', 'Start', 'End', 'Cardholder Type', 'Cardholder', 'Beneficiary','PhilHealth Benefit','Pre-Existing Condition', 'Remarks', 'Benefit Set Name');
...
foreach ($patients as $value => $key)
{
	...
	$this->table->add_row("<font color=".$color.">".$count++.".".$selMulti, anchor(base_url()."records/members/view/".$key['id']."/", "<font color=".$color.">".$key['lastname'].", ".$key['firstname']." ".$key['middlename'], array('target'=>'_blank')),$compinsCI, "<font color=".$color.">".$key['status'], "<font color=".$color.">".mdate('%M %d, %Y', mysql_to_unix($key['dateofbirth'])), "<font color=".$color.">".computeAge($key['dateofbirth']), 
