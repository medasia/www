...
<center><h2>Statement of Account</h2></center><br><br>
...
<?php
	...
	$claims = array(
			array($this->table->add_row('CLAIMS NO: ','<b>'.$details[0]['claims_code'].'</b>')),
			array($this->table->add_row('','')),
			array($this->table->add_row('DATE:','<b>'.mdate('%M %d, %Y',mysql_to_unix($details[0]['print_date']))))
				);
	...
	echo "<table  cellpadding='4' cellspacing='0' class='details_id' style='width:100%'>
			...
			<tr>
				...
				<p>Please find the attachment of the following:<br>
						<b>".$details[0]['attachments']."</b><br></p></td>
				<td align='center' class='details_th' colspan='25%'>".$amount_details."
		...
		$footer = array(
				array($this->table->add_row('','')),
				array($this->table->add_row('','')),
				array($this->table->add_row('Prepared By: <u>   '.$details[0]['prepared_by'].'   </u>','Checked By: <u>   '.$details[0]['checked_by'].'   </u>'))
					);
