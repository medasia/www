...
<?php
	...
	$insurance = array(
				array($this->table->add_row('<b>'.$insurance[0]['name'].'</b>')),
				array($this->table->add_row($insurance[0]['Address']))
				);
	$insu_table = $this->table->generate($insurance);

	//INVOICE table
	$invoice_table = '<table cellpadding="5" cellspacing="0" align="right">';
	$invoice_table.=	'<tr>
							<td align="center" class="invoice_td">Date</td>
							<td align="center" class="invoice_td">Invoice Number</td>
						</tr>';
	$invoice_table.=	'<tr>
							<td align="center" class="invoice_td"><b>'.mdate('%M %d, %Y',mysql_to_unix($topsheet[0]['date'])).'</b></td>
							<td align="center" class="invoice_td"><b>'.$topsheet[0]['invoice_number'].'</b></td>
						</tr>';
	$invoice_table.=	'<tr>
	...
	//AVAILMENTS table
	$count = 1;
	$availments_table = '<table border="1" cellpadding="5" cellspacing="0" style="width:100%">
							<tr>
								<th>No.</th>
								<th>Date Recorded</th>
								<th>Bill #</th>
